...
public class BookDAO {
    ...
    public static ObservableList<Book> getAllBooks() {
        ...
        String query = "SELECT b.book_id, b.title, c.category_name, a.author_name, p.pub_name, b.publish_year, b.quantity, b.status " +
                "FROM books b " +
    ...
    /* Add */
    public static boolean addBook(String title, String authorName, String publisherName,
                                   String categoryName, int publishYear, int quantity) {
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            // 4. Insert sách — status dùng 'ACTIVE' theo ENUM của DB
            String insertSql = "INSERT INTO books (title, author_id, pub_id, category_id, publish_year, quantity, status) "
                    + "VALUES (?, ?, ?, ?, ?, ?, 'ACTIVE')";
    ...
    // Tạo mới nếu chưa có thông tin trong CSDL
